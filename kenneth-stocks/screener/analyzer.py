...
def _format_stock_context(stock) -> str:
    ...
    lines = [
        ...
        f"  FCF yield: {stock.fcf_yield:+.1f}%" if stock.fcf_yield is not None else "  FCF yield: N/A",
        f"  Short interest MoM: {stock.short_interest_chg:+.0f}%" if stock.short_interest_chg is not None else "",
        f"  EPS revisjoner (7d): ↑{stock.eps_revisions_up_7d or 0} ↓{stock.eps_revisions_down_30d or 0}" if stock.eps_revisions_up_7d is not None else "",
        f"  Neste kvartalsrapport: om {stock.next_earnings_days} dager" if stock.next_earnings_days is not None else "",
